					...
					<c:forEach items="${productList}" var="product" varStatus="status">
						<tr>
							<c:if test="${cart.productId eq product.id}">
								...
								<td>${cart.count}</td>
								<td id="${status.count}" class="price" data-product-name="${product.productName}"  data-cart-count="${cart.count}" data-product-price="${product.sellingPrice * cart.count}" data-product-id="${product.id}">${product.sellingPrice} * ${cart.count} = ${product.sellingPrice * cart.count}</td>
							</c:if>
						</tr>
					</c:forEach>
				</c:forEach>
